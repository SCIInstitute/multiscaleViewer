...
void setDetailsForEachVolumeAsSpecifiedInFile(vtkCubeSource* box,
...
int setupAndRunVtkEnvironment(std::string pathOffset)
{
  ...
  try {
    ...
    std::vector<vtkSmartPointer<vtkCubeSource> > box(numBoxes);
//    std::vector<vtkSmartPointer<vtkOutlineFilter> > outlineData(numBoxes);
    ...
    {
      box[i] = vtkSmartPointer<vtkCubeSource>::New();
...
//      setupOutlineFilterForSpecificVolume(outlineData[i], box[i]);
      setupPolyDataMapperForSpecificVolume(mapOutline[i], box[i]);
